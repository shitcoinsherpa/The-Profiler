...
LIWC_CATEGORIES = {
    ...
    # ==================== NCI/CHASE HUGHES DECEPTION INDICATORS ====================
    "minimizing_words": LIWCCategory(
        name="Minimizing Language",
        description="Severity minimizers - softening language that may indicate guilt (Chase Hughes)",
        words=["just", "only", "a little", "a bit", "sort of", "kind of", "somewhat",
               "slightly", "barely", "hardly", "merely", "simply", "basically",
               "pretty much", "more or less", "not really", "not that", "not very",
               "almost", "nearly", "practically"]
    ),
    "severity_softeners": LIWCCategory(
        name="Severity Softeners",
        description="Words that downplay severity - common in deceptive narratives (Chase Hughes)",
        words=["hurt", "touched", "pushed", "took", "borrowed", "disagreement",
               "misunderstanding", "accident", "incident", "situation", "issue",
               "problem", "thing", "stuff", "whatever", "somehow"]
    ),
    "distancing_language": LIWCCategory(
        name="Distancing Language",
        description="Language creating psychological distance from actions (Chase Hughes)",
        words=["one", "someone", "people", "they", "it", "that", "the situation",
               "what happened", "the thing", "stuff", "whatever"],
        patterns=[r"\bmistakes were\b", r"\bit was\b", r"\bthings happened\b"]
    ),
    "denial_intensifiers": LIWCCategory(
        name="Denial Intensifiers",
        description="Unsolicited truth assertions - paradoxically suspicious (Chase Hughes)",
        words=["honestly", "truthfully", "frankly", "seriously", "literally",
               "absolutely", "definitely", "certainly", "truly", "really",
               "i swear", "i promise", "believe me", "trust me", "to be honest",
               "to tell the truth", "i would never", "i didn't", "i never"]
    ),
    "hedge_qualifiers": LIWCCategory(
        name="Hedge Qualifiers",
        description="Hedging and qualifying language - creating deniability (Chase Hughes)",
        words=["i think", "i believe", "i guess", "i suppose", "maybe", "perhaps",
               "possibly", "probably", "likely", "might", "could", "would",
               "seems", "appears", "looks like", "as far as i know",
               "to my knowledge", "if i recall", "if i remember"]
    ),
...
def calculate_deception_indicators(category_results: Dict, total_words: int) -> Dict[str, float]:
    """
    Calculate deception indicator scores based on linguistic research and NCI methodology.
    ...
    Chase Hughes/NCI additions:
    - Minimizing language (severity softeners)
    - Distancing language
    - Denial intensifiers (paradoxically suspicious)
    - Hedge qualifiers
    """
    ...
    # NCI/Chase Hughes indicators
    minimizing_percentage = category_results.get('minimizing_words', {}).get('percentage', 0)
    severity_softener_percentage = category_results.get('severity_softeners', {}).get('percentage', 0)
    distancing_percentage = category_results.get('distancing_language', {}).get('percentage', 0)
    denial_intensifier_percentage = category_results.get('denial_intensifiers', {}).get('percentage', 0)
    hedge_percentage = category_results.get('hedge_qualifiers', {}).get('percentage', 0)

    return {
        # Traditional LIWC indicators
        ...
        # NCI/Chase Hughes indicators
        'minimizing_language': minimizing_percentage * 8,  # High weight - strong indicator
        'severity_softening': severity_softener_percentage * 10,  # Very high weight
        'distancing_behavior': distancing_percentage * 6,
        'denial_intensification': denial_intensifier_percentage * 7,  # Paradoxically suspicious
        'hedge_overuse': hedge_percentage * 4,
...
def generate_analysis_summary(
    ...
    # NCI/Chase Hughes indicators
    summary_parts.append("")
    summary_parts.append("NCI/CHASE HUGHES DECEPTION MARKERS:")
    if deception_indicators.get('minimizing_language', 0) > 20:
        summary_parts.append("  >> FLAG: Minimizing language detected (softening severity)")
    if deception_indicators.get('severity_softening', 0) > 15:
        summary_parts.append("  >> FLAG: Severity softeners present (downplaying actions)")
    if deception_indicators.get('distancing_behavior', 0) > 20:
        summary_parts.append("  >> FLAG: Distancing language (psychological distance from actions)")
    if deception_indicators.get('denial_intensification', 0) > 25:
        summary_parts.append("  >> FLAG: Denial intensifiers (unsolicited truth assertions)")
    if deception_indicators.get('hedge_overuse', 0) > 15:
        summary_parts.append("  >> FLAG: Excessive hedging (creating deniability)")

    # NCI-specific summary
    nci_score = sum([
        deception_indicators.get('minimizing_language', 0),
        deception_indicators.get('severity_softening', 0),
        deception_indicators.get('distancing_behavior', 0),
        deception_indicators.get('denial_intensification', 0),
        deception_indicators.get('hedge_overuse', 0)
    ]) / 5
    summary_parts.append(f"  NCI Linguistic Deception Score: {nci_score:.1f}/100")
    summary_parts.append("")
